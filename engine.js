// ============================================================
// THE TIPSTER EDGE — Engine v8 (Sofascore Edition)
// ============================================================
...
const MIN_CONFIDENCE     = 70;  // Football minimum (probability-based scale)
const NBA_MIN_CONFIDENCE = 65;  // NBA — lower due to confidence cap + single book
const NHL_MIN_CONFIDENCE = 65;  // NHL — lower due to confidence cap + single book
...
const ODDS_MIN          = 1.40;
const ODDS_CORE_MAX     = 2.20;
const ODDS_ELITE_MAX    = 2.50;
...
// ═══════════════════════════════════════════════════════════════
// CONFIDENCE SCORE — how likely is this tip to WIN
// Deliberately independent of edge (which measures value, not outcome)
//
// Signals:
//   1. Model probability     — primary signal: what does the model think?
//   2. Market agreement      — does the market broadly agree with us?
//      Small divergence = both models agree = more confident
//      Large divergence = someone is very wrong = less confident
//   3. Data completeness     — how many games built the stats?
//   4. Market efficiency     — tight margin = well-traded market = reliable price
//   5. Sport-specific cap    — NBA/NHL capped at 80 without lineup/injury data
//
// Output: integer 50–92
// 50–64 = uncertain (below publication threshold)
// 65–74 = moderate confidence
// 75–84 = good confidence (typical published range)
// 85–92 = high confidence (rare, requires strong model + market agreement)
// ═══════════════════════════════════════════════════════════════

function confidenceFromSignals({ modelProb, trueImplied, dataQualityTier, sport, gamesPlayed, bookCount, avgMargin }) {

  // ── Signal 1: Model probability → base confidence ─────────
  // The model's raw win probability is the most direct measure
  // of how likely the tip is to win. Anchored to probability bands.
  //
  // Note: we use the SELECTION'S probability (the outcome we're tipping),
  // so higher modelProb = more likely to win = higher base confidence.
  let base;
  if      (modelProb >= 0.72) base = 84;
  else if (modelProb >= 0.65) base = 80;
  else if (modelProb >= 0.58) base = 76;
  else if (modelProb >= 0.52) base = 72;
  else if (modelProb >= 0.47) base = 68;
  else                        base = 62;

  // ── Signal 2: Market agreement ────────────────────────────
  // If our model probability is close to the market's implied probability,
  // both independent pricing sources agree on the likely outcome.
  // Large divergence means uncertainty — one of us is wrong.
  const divergence = Math.abs(modelProb - (trueImplied || modelProb));
  let marketAdj;
  if      (divergence <= 0.03) marketAdj =  3;  // strong agreement
  else if (divergence <= 0.06) marketAdj =  1;  // mild agreement
  else if (divergence <= 0.10) marketAdj =  0;  // neutral
  else if (divergence <= 0.15) marketAdj = -2;  // mild disagreement
  else                         marketAdj = -5;  // large disagreement — uncertain

  // ── Signal 3: Data completeness ───────────────────────────
  // More games = more reliable stats = more confident model probability
  const games = gamesPlayed || 0;
  let dataAdj;
  if      (games >= 25) dataAdj =  2;
  else if (games >= 15) dataAdj =  0;
  else if (games >= 8)  dataAdj = -2;
  else if (games >= 4)  dataAdj = -4;
  else                  dataAdj = -8;  // very few games — model unreliable

  // Apply data quality tier penalty on top
  if (dataQualityTier === 0.7) dataAdj -= 3;
  else if (!dataQualityTier || dataQualityTier === 0) dataAdj -= 8;

  // ── Signal 4: Market efficiency ───────────────────────────
  // Tighter bookmaker margin = more efficiently priced market
  // = market implied probability is more trustworthy
  // avgMargin is in %, e.g. 5.2 means 5.2% overround
  const margin = avgMargin || 10;
  let efficiencyAdj;
  if      (margin <= 3)  efficiencyAdj =  1;
  else if (margin <= 6)  efficiencyAdj =  0;
  else if (margin <= 10) efficiencyAdj = -1;
  else                   efficiencyAdj = -2;

  // ── Combine ───────────────────────────────────────────────
  let conf = base + marketAdj + dataAdj + efficiencyAdj;
  conf = Math.max(50, Math.min(92, Math.round(conf)));

  // ── Sport-specific caps ───────────────────────────────────
  // NBA/NHL: no confirmed lineup/injury/goalie data → cap at 80
...
async function analyseFootballFixture(event, sport) {
  try {
    ...
    const candidates = [];
    const h2hEdgeMin = MIN_EDGE_PCT + FOOTBALL_EDGE_PREMIUM;
    const eliteH2H   = ELITE_H2H_EDGE + FOOTBALL_EDGE_PREMIUM;

    // Home win
    if (market.homeOdds >= ODDS_MIN && market.homeOdds <= ODDS_ELITE_MAX && market.trueHome > 0) {
      ...
      if (edge >= h2hEdgeMin) {
        ...
        const conf  = confidenceFromSignals({ modelProb: homeWin, trueImplied: market.trueHome, dataQualityTier: 1.0, sport: 'Football', gamesPlayed: games, bookCount: market.bookCount, avgMargin: market.avgMargin });
    ...
    // Away win
    if (market.awayOdds >= ODDS_MIN && market.awayOdds <= ODDS_ELITE_MAX && market.trueAway > 0) {
      ...
      if (edge >= h2hEdgeMin) {
        ...
        const conf  = confidenceFromSignals({ modelProb: awayWin, trueImplied: market.trueAway, dataQualityTier: 1.0, sport: 'Football', gamesPlayed: games, bookCount: market.bookCount, avgMargin: market.avgMargin });
    ...
    if (market.drawOdds >= 2.50 && draw > 0.22 && market.trueDraw > 0) {
      ...
      if (edge >= h2hEdgeMin + 5) {
        ...
        const games = Math.min(hStats?.homeGames || 0, aStats?.awayGames || 0);
        const conf  = confidenceFromSignals({ modelProb: draw, trueImplied: market.trueDraw, dataQualityTier: 1.0, sport: 'Football', gamesPlayed: games, bookCount: market.bookCount, avgMargin: market.avgMargin });
...
async function analyseNHLFixture(event, sport) {
  try {
    ...
    let lH = Math.max(0.5, Math.min(6.0, homeAttack * awayDef * NHL_LEAGUE_AVG_GF + NHL_HOME_ADVANTAGE));
    let lA = Math.max(0.5, Math.min(6.0, awayAttack * homeDef * NHL_LEAGUE_AVG_GF));
    ...
    if (market.homeOdds >= 1.25 && market.homeOdds <= 4.5 && market.trueHome > 0) {
      ...
      if (edge >= NHL_MIN_EDGE) {
        const conf  = confidenceFromSignals({ modelProb: homeWinML, trueImplied: market.trueHome, dataQualityTier: 1.0, sport: 'Ice Hockey', gamesPlayed: homeStats.gamesPlayed, bookCount: market.bookCount, avgMargin: market.avgMargin });
        ...
        if (stake > 0 && conf >= NHL_MIN_CONFIDENCE && falseEdgeCheck(c, market)) candidates.push(c);
      }
    }
    ...
    if (market.awayOdds >= 1.25 && market.awayOdds <= 4.5 && market.trueAway > 0) {
      ...
      if (edge >= NHL_MIN_EDGE) {
        const conf  = confidenceFromSignals({ modelProb: awayWinML, trueImplied: market.trueAway, dataQualityTier: 1.0, sport: 'Ice Hockey', gamesPlayed: awayStats.gamesPlayed, bookCount: market.bookCount, avgMargin: market.avgMargin });
    ...
    if (market.over25Odds >= 1.50 && market.over25Odds <= 2.40) {
      ...
      if (edge >= NHL_MIN_EDGE) {
        const conf  = confidenceFromSignals({ modelProb: over55, trueImplied: over55IP, dataQualityTier: 1.0, sport: 'Ice Hockey', gamesPlayed: Math.min(homeStats.gamesPlayed, awayStats.gamesPlayed), bookCount: market.bookCount, avgMargin: market.avgMargin });
  ...
  try {
    ...
    if (market.homeOdds >= 1.25 && market.homeOdds <= 5.0 && market.trueHome > 0) {
      ...
      if (edge >= NBA_MIN_EDGE) {
        const conf  = confidenceFromSignals({ modelProb: homeWinP, trueImplied: market.trueHome, dataQualityTier: 1.0, sport: 'Basketball', gamesPlayed: homeStats.gamesPlayed, bookCount: market.bookCount, avgMargin: market.avgMargin });
        ...
        if (stake > 0 && conf >= NBA_MIN_CONFIDENCE && falseEdgeCheck(c, market)) candidates.push(c);
      }
    }
    ...
    if (market.awayOdds >= 1.25 && market.awayOdds <= 5.0 && market.trueAway > 0) {
      ...
      if (edge >= NBA_MIN_EDGE) {
        const conf  = confidenceFromSignals({ modelProb: awayWinP, trueImplied: market.trueAway, dataQualityTier: 1.0, sport: 'Basketball', gamesPlayed: awayStats.gamesPlayed, bookCount: market.bookCount, avgMargin: market.avgMargin });
...
function applyStrictRules(tip, existingBestOdds = null) {
  ...
  // Stake from confidence + grade (calibrated to new probability-based scale)
  const conf = parseFloat(tip.confidence || 0);
  let stake;
  if (conf < 72)       { stake = 1.0; }
  else if (conf < 78)  { stake = grade === 'A+' ? 2.0 : 1.5; }
...
async function saveTips(tips) {
  ...
  for (const tip of tips) {
    try {
      ...
      const { data: existing } = await supabase.from('tips').select('id, tip_ref, confidence, odds, best_odds, bookmaker, status')
...
async function fetchSofascoreResult(eventId) {
  ...
  const e = data.event;
  if (e.status?.type !== 'finished' && e.status?.description !== 'Ended') return null;
  const result = {
    homeScore: e.homeScore?.current ?? e.homeScore?.normaltime ?? null,
    awayScore: e.awayScore?.current ?? e.awayScore?.normaltime ?? null,
    finished:  true,
  };
  if (result.homeScore !== null) scoreCache[eventId] = result;
  return result.homeScore !== null ? result : null;
}
  ...
  for (const tip of pending) {
    try {
      ...
      if (eventId) {
        ...
        const cachedEvent  = cachedEvents.find(e =>
          nameMatch(e.home_team, tip.home_team) && nameMatch(e.away_team, tip.away_team)
        );
        if (!cachedEvent?.id) { console.log(`⏳ No cached event for: ${tip.home_team} vs ${tip.away_team}`); continue; }
...
async function runEngine() {
  console.log(`\n🚀 Engine v8 — ${new Date().toLocaleString('en-GB', { timeZone: 'Europe/London' })}`);
...
(async () => {
  console.log(`\n🟢 The Tipster Engine v8 starting...`);
